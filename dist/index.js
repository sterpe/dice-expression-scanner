...
function isDiceExpressionOrValue(S) {
  return (/^([1-9][0-9]*)?(((d|D)?[1-9][0-9]*)|(d\%|D\%|0))$/.test(S)
  );
}
...
DiceExpressionScanner.prototype.nextToken = function (source) {
  ...
  if (/^(?:\+|\-)/.test(source.currentChar)) {
    return [new OperatorToken(currentPosition, source), source.nextChar()].shift();
  }
  ...
  if (isOk(S)) {
    var T = isDiceExpression(S) ? DiceExpressionToken : ValueToken;
